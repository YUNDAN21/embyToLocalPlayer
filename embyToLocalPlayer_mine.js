// ==UserScript==
// @name         embyToLocalPlayer_mine
// @name:zh-CN   embyToLocalPlayer_mine
// @name:en      embyToLocalPlayer_mine
...
(function () {
    ...
    let sortParamsCache = {}; // {containerParentId: {SortBy: '...', SortOrder: '...'}}
    let lastItemsRequestParentId = null;
    ...
    async function dealWithPlaybackInfo(raw_url, url, options) {
        ...
        if (mainEpInfo?.Type == 'MusicAlbum') {
            alert('etlp: Cannot play MusicAlbum directly. Please select a Song instead.');
            return false;
        }
    ...
    async function dealWithPlaylistOrAlbum(item, containerParentId) {
        // 处理播放列表或专辑：获取其中的所有歌曲
        let itemId = item.Id;
        let userId = ApiClient._serverInfo.UserId;
        
        let playlistItemsInfo = [];
        try {
            logger.info(`Fetching items from Playlist/Album: ${itemId}`);
            const options = {
                'Fields': 'Id,Name,Type,Album,AlbumId,Artist,RunTimeTicks',
                'Limit': 5000,
                'Recursive': true,
            };

            // 对于 Playlist，获取其中的歌曲；对于 Album，获取其中的歌曲
            let playlistResp = await ApiClient.getItems(userId, {
                ParentId: itemId,
                ...options
            });

            if (playlistResp && playlistResp.Items && playlistResp.Items.length > 0) {
                logger.info(`✓ Playlist/Album returned ${playlistResp.Items.length} items`);
                playlistItemsInfo = playlistResp.Items;
            } else {
                logger.error(`Playlist/Album is empty for itemId=${itemId}`);
            }
        } catch (error) {
            logger.error('Failed to fetch Playlist/Album items:', error);
        }

        let [mainEpInfo, playbackData] = await Promise.all([
            getItemInfoWithCace(itemId),
            getPlaybackWithCace(itemId),
        ]);

        let positonTicks = mainEpInfo.UserData?.PlaybackPositionTicks || 0;
        ...
        let extraData = {
            mainEpInfo: mainEpInfo,
            episodesInfo: [],
            playlistInfo: playlistItemsInfo,
            gmInfo: GM_info,
            userAgent: navigator.userAgent,
        };

        if (playlistItemsInfo.length > 0) {
            logger.info(`Sending ${playlistItemsInfo.length} Playlist items to backend`);
        }

        embyToLocalPlayer(playbackUrl, {}, playbackData, extraData);
    }

    async function handleListPlay(_container, isShuffle = false) {
        const containerParentId = lastItemsRequestParentId;
        if (!containerParentId) {
            logger.info('🎬 List play clicked, but containerParentId not found.');
            return;
        }
        ...
        if (containerParentId in sortParamsCache) {
            const cachedSort = sortParamsCache[containerParentId];
            options.SortBy = cachedSort.SortBy;
            options.SortOrder = cachedSort.SortOrder;
            logger.info(`✓ Using cached sort for containerParentId=${containerParentId}:`, cachedSort.SortBy, cachedSort.SortOrder);
        } else {
            logger.info(`ℹ No cached sort for containerParentId=${containerParentId}, using API default`);
        }

        const playlistResp = await ApiClient.getItems(userId, {
            ParentId: containerParentId,
            ...options
        });

        let playlistItemsInfo = playlistResp?.Items || [];
        if (!playlistItemsInfo.length) {
            logger.error(`API returned empty for containerParentId=${containerParentId}`);
            return;
        }

        if (isShuffle) {
            for (let i = playlistItemsInfo.length - 1; i > 0; i--) {
                const j = Math.floor(Math.random() * (i + 1));
                [playlistItemsInfo[i], playlistItemsInfo[j]] = [playlistItemsInfo[j], playlistItemsInfo[i]];
            }
            logger.info('🎬 Shuffled playlist for random play.');
        }

        const firstItem = playlistItemsInfo[0];
        const [mainEpInfo, playbackData] = await Promise.all([
            getItemInfoWithCace(firstItem.Id),
            getPlaybackWithCace(firstItem.Id),
        ]);

        const positonTicks = mainEpInfo?.UserData?.PlaybackPositionTicks || 0;
        ...
        const baseUrl = `${window.location.origin}/emby/Items/${firstItem.Id}/PlaybackInfo`;
        ...
        if (playlistItemsInfo.length > 0) {
            logger.info(`Sending ${playlistItemsInfo.length} playlist items to backend，${playlistItemsInfo[0]}`);
        }

        embyToLocalPlayer(playbackUrl, {}, playbackData, extraData);
    }

    async function deailWithItemInfo(item, itemList = null, itemIndex = 0, containerParentId = null) {
        let itemId = item.Id;
        ...
        // 如果有 containerParentId，使用 API 获取完整播放列表
        let playlistItemsInfo = [];
        if (containerParentId && item.Type === 'Audio') {
            logger.info(`Fetching full playlist using containerParentId: ${containerParentId}`);
            try {
                const options = {
                    'Fields': 'Id,Name,Type,Album,AlbumId,Artist,RunTimeTicks',
                    'Limit': 5000,
                    'Recursive': true,
                    'IncludeItemTypes': 'Audio',  // 只获取音频，排除 Playlist 等其他类型
                };
                ...
                if (playlistResp && playlistResp.Items && playlistResp.Items.length > 0) {
                    logger.info(`✓ API returned ${playlistResp.Items.length} items for containerParentId=${containerParentId}`);
                    // 获取完整列表，后端会根据 mainEpInfo 定位当前项目
                    playlistItemsInfo = playlistResp.Items;
                    logger.info(`Full playlist: ${playlistItemsInfo.length} items, first item: ${playlistItemsInfo[0]?.Name}`);
                } else {
                    logger.error(`API returned empty for containerParentId=${containerParentId}`);
                }
            } catch (error) {
    ...
    document.addEventListener('click', e => {
        ...
        const listPlayBtn = e.target.closest('button.btnPlay,button.btnShuffle,button.cardOverlayButton-primary[data-action="resume"]');
        const cardPlayBtn = e.target.closest('button.cardOverlayFab-primary[data-action="play"], button.listItemOverlayButton-imagehover[data-action="playallfromhere"], button.cardOverlayFab-primary[data-action="resume"],.listItemBody');
            ...
            (async () => {
                try {
                    ...
                    if (isShuffle) {
                        ...
                        await handleListPlay(container, false);
                    }
                } catch (error) {
                    logger.error('List play failed:', error);
                }
            })();
            return;
        }
        ...
        const parentCard = e.target.closest('.virtualScrollItem.card, .backdropCard[data-index], .virtualScrollItem.listItem');
        if (!parentCard) {
            logger.info('🎬 Play button clicked, but parent card not found.');
            return;
        }
        ...
        // 从 container 元素获取 ParentId（播放列表或媒体库）
        const containerParentId = container.dataset.monitorids || container.dataset.parentid;

        logger.info(`🎬 Container info: itemList.length=${itemList.length}, containerParentId=${containerParentId}`);

        if (!['Movie', 'Episode', 'Audio', 'MusicAlbum', 'Song', 'Playlist'].includes(itemType)) {
            logger.info(`🎬 Play button clicked, but ${itemType} not within legal itemType.`);
            return
        }
        ...
        // 处理播放列表或专辑：获取其中的所有歌曲
        if (itemType === 'Playlist' || itemType === 'MusicAlbum') {
            dealWithPlaylistOrAlbum(item, containerParentId);
        } else if (itemType === 'Audio' && containerParentId) {
            // 媒体库歌曲页面点击播放，传递 ParentId 以获取完整列表
            deailWithItemInfo(item, itemList, index, containerParentId);
        } else {
            // 处理单个媒体项
    ...
    unsafeWindow.fetch = async (input, options) => {
        ...
        // 拦截 Items 请求，提取排序参数
        if (urlStr.includes('Items?') && urlStr.includes('ParentId=')) {
            const parentIdMatch = urlStr.match(/ParentId=([a-zA-Z0-9]+)/);
            const sortByMatch = urlStr.match(/SortBy=([^&]+)/);
            const sortOrderMatch = urlStr.match(/SortOrder=([^&]+)/);
            
            if (parentIdMatch && sortByMatch) {
                const parentId = parentIdMatch[1];
                lastItemsRequestParentId = parentId;
                const sortBy = decodeURIComponent(sortByMatch[1]);
                const sortOrder = sortOrderMatch ? decodeURIComponent(sortOrderMatch[1]) : 'Descending';
                
                sortParamsCache[parentId] = { SortBy: sortBy, SortOrder: sortOrder };
                logger.debug(`📊 Cached sort params for parentId=${parentId}:`, sortBy, sortOrder);
            }
        }
